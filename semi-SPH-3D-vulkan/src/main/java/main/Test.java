...
import static graph.AlignmentUtils.alignas;
import static graph.AlignmentUtils.alignof;
...
public class Test {
	...
	private static final Vertex[] VERTICES = {
		new Vertex(new Vector2f(-0.5f, -1.5f), new Vector3f(1.0f, 0.0f, 0.0f)),
	...
	private long computeDescriptorSetLayout;
	private List<Long> computeDescriptorSet;
	...
	private List<VkCommandBuffer> computeCommandBuffers;
...
//            }
	...
	private void createGraphicsPipeline() {
		...
		SPIRV vertShaderSPIRV = compileShaderFile("shaders/vertex.vs", VERTEX_SHADER);
		SPIRV fragShaderSPIRV = compileShaderFile("shaders/frag.fs", FRAGMENT_SHADER);
	...
	void createComputePipeline() {
	    try (MemoryStack stack = stackPush()) {
		SPIRV compShaderSPIRV = compileShaderFile("shaders/comp.txt", COMPUTE_SHADER);

		long compShaderModule = createShaderModule(compShaderSPIRV.bytecode());

		ByteBuffer entryPoint = stack.UTF8("main");

		VkPipelineShaderStageCreateInfo.Buffer computeShaderStageInfo = VkPipelineShaderStageCreateInfo
			.calloc(1, stack);
		...
		pipelineLayoutInfo.pSetLayouts(stack.longs(computeDescriptorSetLayout));
		...
		if (vkCreateComputePipelines(device, VK_NULL_HANDLE, pipelineInfo, null,
	...
	private void createComputeBuffer() {
		...
		createBuffer(bufferSize,
			...
			VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, pBuffer, pBufferMemory);
...
//	    Device d = new Device(instance, physicalDevice, device, graphicsQueue, presentQueue, commandPool);
//	    mvp = new ModelViewProj(WIDTH,HEIGHT);
//	    mvp.gira();
//
//	    ubo = new UniformBuffer(d, mvp.MVPData(), mvp.getLayout());
	}

	private void createComputeDescriptorSetLayout() {
	    try (MemoryStack stack = stackPush()) {
		VkDescriptorSetLayoutBinding.Buffer layoutBindings = VkDescriptorSetLayoutBinding.calloc(3, stack);

		VkDescriptorSetLayoutBinding[] particles = { layoutBindings.get(0), layoutBindings.get(1),
			layoutBindings.get(2) };

		particles[0].binding(0);
		particles[0].descriptorCount(1);
		particles[0].descriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
		particles[0].pImmutableSamplers();
		particles[0].stageFlags(VK_SHADER_STAGE_COMPUTE_BIT);

		particles[1].binding(1);
		particles[1].descriptorCount(1);
		particles[1].descriptorType(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		particles[1].pImmutableSamplers();
		particles[1].stageFlags(VK_SHADER_STAGE_COMPUTE_BIT);

		particles[2].binding(2);
		particles[2].descriptorCount(1);
		particles[2].descriptorType(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		particles[2].pImmutableSamplers();
		particles[2].stageFlags(VK_SHADER_STAGE_COMPUTE_BIT);
		...
		computeDescriptorSetLayout = pDescriptorSetLayout.get(0);
	...
	private void createDescriptorSets() {
		...
		for (int i = 0; i < pDescriptorSets.capacity(); i++) {
		    ...
		    //ubo.nextLayout(i);
		    //bufferInfo.buffer(ubo.getBufferPointer());
	...
	private void createComputeCommandBuffers() {
	    ...
	    computeCommandBuffers = new ArrayList<>(commandBuffersCount);
	    try (MemoryStack stack = stackPush()) {
		...
		for (int i = 0; i < commandBuffersCount; i++) {
		    computeCommandBuffers.add(new VkCommandBuffer(pCommandBuffers.get(i), device));
		}
	    }
	}
...
//
//	private void test() {
//	    DefaultVulkanSetup vulkan = new DefaultVulkanSetup(instance, window, surface, debugMessenger);
//	    Device d = new Device(instance, physicalDevice, device, graphicsQueue, presentQueue, commandPool);
//	    framebuffer = new FrameBuffer(d, vulkan, swapChain, swapChainImages, swapChainImageFormat, swapChainExtent,
//		    swapChainImageViews, swapChainFramebuffers, renderPass, null, null);
//	    
//	    //framebuffer= new FrameBuffer(d, vulkan);
//
...
//	    ubo = new UniformBuffer(d, mvp.MVPData(), mvp.getLayout());
//
//	    vb = new VertexBuffer(d, vertices, verticesLayout);
//	    ib = new IndexBuffer(d, indexs);
//	    
//
//	    pip = new Pipeline(d, framebuffer, descriptorSetLayout, pipelineLayout, graphicsPipeline, descriptorPool,
//		    descriptorSets, commandBuffers);
//
//	    vertexShader = new VertexShader("shaders/vertex.vs", d);
//	    fragmentShader = new FragmentShader("shaders/frag.fs", d);
//
//	    vertexShader.addUniform(ubo, 0);
//	    vertexShader.setIndex(ib);
//	    vertexShader.setVertex(vb);
//
//	    pip.shaders.add(vertexShader);
//	    pip.shaders.add(fragmentShader);
//	    
//	    pip.createGraphicsPipeline(vertexShader, fragmentShader, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, false);
//
//	    pip.createDescriptorPool();
//
//	    pip.createDescriptorSets();
//
//	    pip.createGraphCommandBuffers(vertexShader);
//
//	}
	...
	private void createCommandBuffers() {
		...
		for (int i = 0; i < commandBuffersCount; i++) {
		    ...
		    System.out.println("f " + swapChainFramebuffers.get(i));
			...
			//LongBuffer vertexBuffers = stack.longs(vb.getBufferPointer());
			LongBuffer offsets = stack.longs(0);
			vkCmdBindVertexBuffers(commandBuffer, 0, vertexBuffers, offsets);

			vkCmdBindIndexBuffer(commandBuffer, indexBuffer, 0, VK_INDEX_TYPE_UINT16);
			//vkCmdBindIndexBuffer(commandBuffer, ib.getBufferPointer(), 0, VK_INDEX_TYPE_UINT16);
	...
	private void recordComputeCommandBuffer(VkCommandBuffer commandBuffer) {
		...
		vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 0,
			stack.longs(computeDescriptorSet.get(currentFrame)), null);
		vkCmdDispatch(commandBuffer, 10000 / 256, 1, 1);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	void recordCommandBuffer(VkCommandBuffer commandBuffer, int imageIndex) {
		...
		vkCmdBeginRenderPass(commandBuffer, renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
		{
		    ...
		    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0,
			    stack.longs(descriptorSets.get(currentFrame)), null);
	...
	private void createSyncObjects() {
	    ...
	    //framebuffer.setImagesInFlight(imagesInFlight);
	    //framebuffer.setInFlightFrames(inFlightFrames);
	}
...
//
//	    framebuffer.test(pip.getCommandBuffer());
//
//	    
//	    try {
//		Thread.sleep(500);
...
//	    }
		...
		//System.out.println(mvp.getModel());
